...
const SendSPLTokenBtn:FC = () => {
    ...
    let mintUSDCAdress = 'EPjFWdd5AufqSSqeM2qN1xzybapC8G4wEGGkZwyTDt1v' //Адрес токена ,который нужно переслать
    let theWallet = 'A8grZ1aaL9Hm8sC7mtVyiAqdkFf4mB63aBpfq2WR9drt' //Кошелек, куда пересылать
    ...
    const onClick = useCallback( async (e:any) => {
        try {
            ...
            const mint1 = new PublicKey(mintUSDCAdress) //Адрес токена ,который нужно переслать
    ...
    return (
        <button value={0.1}  onClick={onClick}>SEND 0.1 SPL TOKEN USDC</button>  
    );
};
